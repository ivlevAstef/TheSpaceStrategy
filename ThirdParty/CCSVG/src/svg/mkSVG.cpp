...
//		}
  ...
  uint32_t SVG::string_hex_color_to_uint( std::string& hexstring ) {
...
//			}
  ...
  void SVG::parse_path_transform( std::string& tr )	{
  ...
  void SVG::parse_path_d( std::string& d ) {
  ...
  void SVG::parse_path_style( std::string& ps ) {
    ...
    for (; values_tokens != rend; ++values_tokens) {
      ...
      //cout << *k << ":" << *v << endl;
      style_key_values[k->str()] = v->str();
    }
    
    std::map<std::string, std::string>::iterator kv = style_key_values.find( std::string("fill") );
