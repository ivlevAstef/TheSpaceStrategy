...
namespace MonkVG {
  ...
  bool OpenGLPath::draw( VGbitfield paintModes ) {
    ...
    if ( _fillPaintForPath && (_fillPaintForPath->getPaintType() == VG_PAINT_TYPE_LINEAR_GRADIENT || _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_RADIAL_GRADIENT || _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_RADIAL_2x3_GRADIENT || _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_LINEAR_2x3_GRADIENT) ) {
      glContext.setImageMode( VG_DRAW_IMAGE_NORMAL );  
    }
    

    if( (paintModes & VG_FILL_PATH) && !_vertices.empty() && _fillPaintForPath) {
      // draw
      IContext::instance().fill();

      if ( _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_COLOR ) {
        glContext.vertexPointer(2, GL_FLOAT, 0, (GLvoid*)&_vertices[0]);
        ...
        glContext.bindTexture(_fillPaintForPath->getGradientImage()->name());
        glContext.vertexPointer(2, GL_FLOAT, sizeof(textured_vertex_t), (GLvoid*)&_texturedVertices[0].v);
        glContext.texCoordPointer(2, GL_FLOAT, sizeof(textured_vertex_t), (GLvoid*)&_texturedVertices[0].uv);
      }
      glContext.drawArrays(GL_TRIANGLES, 0, _numberFillVertices);
    ...
    if ( (paintModes & VG_STROKE_PATH) && !_strokeVertices.empty() ) {
      // draw
      IContext::instance().stroke();
      glContext.vertexPointer(2, GL_FLOAT, sizeof(v2_t), (GLvoid*)&_strokeVertices[0]);	
    }
...
#define M_PI 3.14159265358979323846 
  ...
  void OpenGLPath::buildFill() {
    ...
    gluTessCallback( _fillTesseleator, GLU_TESS_BEGIN_DATA  , (GLvoid (APIENTRY *) ( )) &OpenGLPath::tessBeginData );
    gluTessCallback( _fillTesseleator, GLU_TESS_END_DATA    , (GLvoid (APIENTRY *) ( )) &OpenGLPath::tessEndData );
    gluTessCallback( _fillTesseleator, GLU_TESS_VERTEX_DATA , (GLvoid (APIENTRY *) ( )) &OpenGLPath::tessVertexData );
    gluTessCallback( _fillTesseleator, GLU_TESS_COMBINE_DATA, (GLvoid (APIENTRY *) ( )) &OpenGLPath::tessCombineData );
    gluTessCallback( _fillTesseleator, GLU_TESS_ERROR_DATA  , (GLvoid (APIENTRY *) ( )) &OpenGLPath::tessErrorData );
...
//					c[2] = 0;
  ...
  void CALLBACK OpenGLPath::tessBeginData(GLenum type, GLvoid* user) {
  ...
  void CALLBACK OpenGLPath::tessEndData(GLvoid* user) {
    //		OpenGLPath* me = (OpenGLPath*)user;
    //		me->endOfTesselation();
  ...
  void CALLBACK OpenGLPath::tessVertexData(GLvoid* vertex, GLvoid* user) {
    OpenGLPath* me = (OpenGLPath*)user;
  ...
  void CALLBACK OpenGLPath::tessCombineData(GLdouble coords[3], void *data[4],
  ...
  void CALLBACK OpenGLPath::tessErrorData(GLenum errorCode, GLvoid* polygon_data) {
