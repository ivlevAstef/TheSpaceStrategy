...
namespace MonkVG {
  ...
  void OpenGLImage::draw() {
    ...
    if ( IContext::instance().getImageMode() == VG_DRAW_IMAGE_MULTIPLY ) {
      ...
      glContext.setColor4f(color[0], color[1], color[2], color[3]);
    }

    glContext.bindTexture(_name);
    glContext.vertexPointer(3, GL_FLOAT, 0, vertices);
    glContext.texCoordPointer(2, GL_FLOAT, 0, coordinates);
